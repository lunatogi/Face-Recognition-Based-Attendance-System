...
This project is built for **Raspberry Pi** and uses **OpenCV / NumPy + face_recognition** for recognition, plus an **MFRC522 RFID reader** for card verification. 
...
- Only accepts when the face matches the card’s owner with a confidence rule (multiple encodings per person). 
...
- **Remove Worker (Kişi çıkar)** 

### ✅ Exports to Excel (and optional Google Sheets)
- Generates a **daily .xlsx** file with worker details + entry/exit times + computed totals. 
- Updates a master “Puantaj.xlsx” template (stored on external media path in code). 
- Contains code to append to Google Sheets using a service account (credentials JSON in repo). 
...
- `gui.py` – Full-screen Tkinter “control panel” (entry/exit/add/remove/overtime, foreman toggle). 
...
- `person_quit.py` – Remove worker by TC (deletes their 4 encodings & metadata from `database.json`). 
- `pi_face_recognition_main.py` – Entry flow: face + RFID verification, writes daily JSON log. 
- `pi_face_recognition_exit_main.py` – Exit flow: updates exit times and produces Excel exports.
- `card_reader.py` – Minimal RFID read helper (MFRC522). 
...
- Extracts face encodings and stores them into `database.json`. 
...
- Records “scheduled entry time” vs “actual entry time” into a daily JSON file. 
